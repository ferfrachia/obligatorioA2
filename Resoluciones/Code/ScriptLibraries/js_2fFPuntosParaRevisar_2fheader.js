...
function cambiarUnidad(){
	...
	cargarBandejaAsuntosParaRevisar()
}

var grid;

//Función que inicializa la bandeja de sesiones
function cargarBandejaAsuntosParaRevisar(){
	actualizarAltos();
	...
	document.getElementById('BandejaAsuntosParaRevisar').style.overflow = "hidden";
	document.getElementById('BandejaAsuntosParaRevisar').style.width = "0";
	document.getElementById('BandejaAsuntosParaRevisar').style.heigth = "0";
	
	var path = DirABS();

	var strXml = DirABS()+"/GenerarBandejaParaRevisarXMLNew?OpenAgent&estado=ParaRevisar&unidad="+unidad;
	//alert(strXml);
	//var strXml = urlBandejaParaFirmar+ "&RestrictToCategory=EXP_"+unidad+usuario+anio+"&Count=3000";
	var mygrid = new dhtmlXGridObject('BandejaAsuntosParaRevisar');
	mygrid.attachEvent("onXLE", cargarCantidadDocs);
	mygrid.setImagePath("/codebase/imgs/");
	mygrid.setHeader("Número Exp,Fecha,Referencia,Unidad,Supervisor, &nbsp; ",null,[estiloHeader,estiloHeader,estiloHeader,estiloHeader,estiloHeader,estiloHeader]);
	mygrid.setInitWidths("150,150,*,150,150,50");
	...
	grid = mygrid;
	mygrid.setColSorting("str,str,str,str,str, na");
	...
	document.getElementById('BandejaAsuntosParaRevisar').style.overflow = "hidden";
	document.getElementById('BandejaAsuntosParaRevisar').style.width = "100%";
	document.getElementById('BandejaAsuntosParaRevisar').style.heigth = "100%";
	if(navigator.appName == "Netscape")
		document.getElementById('CabezalBandejaAsuntosParaRevisarWrap').style.width = calcularAncho() - 18 +"px";
}
...
function actualizarAltos(){
	...
	if(ALTO_MAXIMIZADA<0)
		ALTO_MAXIMIZADA = 0;
	document.getElementById('CabezalBandejaAsuntosParaRevisarWrap').style.height = ""+ALTO_MAXIMIZADA+"px";
}
