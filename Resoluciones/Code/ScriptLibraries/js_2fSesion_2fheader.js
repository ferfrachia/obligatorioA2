...
{	
	...
	//if (!validar(document.forms[0].fSesion.value,"Debe ingresar la Fecha"))
	//	return false;
	if (!tieneHoraInicio())
		return false;
...
function FirmarPasaraFinalizar(id){
	...
	var firmar = document.getElementsByName("rFirmarAntesFinalizar")[0].value;
	if (firmar=="No"){
		$("#DialogoFirmaEnviarFin").dialog("open");
	} else {
		var pasar = VerificarCantFirmas("FirmarEnviar");
		if (pasar==true)
			$("#DialogoFirmaEnviarFin").dialog("open");
	}
}

function PasaraFirmar(id){
	if(document.forms[0].TextoActa.value=="<P>&nbsp;</P>" || document.forms[0].TextoActa.value==""){
		alert('Debe ingresar el Acta de la sesión');
	}
	else {
		if(document.getElementsByName("RolFirmanteActa")[0].value==""){
			url = DirABS() + "/Usuarios_Rol_Firmantes_Actas?OpenAgent";
			$.ajax({
				type: "GET",
				url: url,
				cache: false,
				...
				success: function(data) {
					document.getElementsByName("RolFirmanteActa")[0].value = data.valores;
					document.getElementsByName("nbMiembrosPub")[0].value = data.valores;
				}
			})
		}
		$("#dialogoAgregarFirmantes").dialog("open");
		//El metodo que llama al controller esta en el SubForm AccAgregarFirmantes
	}
}
...
function MoverAsuntos(id){
	var pathDB = DirABS();
	window.location.href = pathDB+"/ParametrosMoverAsuntos?OpenForm&nrosesion="+document.forms[0].sNroSesion.value+"&id="+id+"&unidad="+"9017,";
}
...
function PasaraFinalizar(id){
	var firmar = document.getElementsByName("rFirmarAntesFinalizar")[0].value;
	//Si es obligatorio firmar consulta por la cantidad de firmas
	if (firmar!="No"){
		var pasar = VerificarCantFirmas("Finalizar");
		if (pasar==true){
			if (document.forms[0].ActaRequeridaEFinalizar.value=="Si"){
				if(document.forms[0].TextoActa.value=="<P>&nbsp;</P>" || document.forms[0].TextoActa.value==""){
					alert('Debe ingresar el Acta de la sesión');
...
//Verifica que hayan firmado la cantidad de usuarios necesaria para pasar a finalizar
function VerificarCantFirmas(accion){
	var resultado = false;
	
	//Obtengo la cantidad de firmas que necesito para enviar a finalizar.
	...
	//Cuento la cantidad de usuarios que firmaron
	var Firmados = 0;
	if (document.getElementsByName("UsuarioFirma")[0]!=undefined){
		var usrsFirmas = document.getElementsByName("UsuarioFirma")[0].value;
		usrsFirmas = usrsFirmas.split("; ");
		Firmados = usrsFirmas.length;
	}
	
	//Si la cantidad de firmas es menor a la cantidad de firmas necesarias retorno false
	if (accion=="FirmarEnviar"){
		if (Firmados+1<cantFirmas){
			alert("Deben firmar " + cantFirmas + " usuarios para enviar a finalizar");
			return false;
		}
	} else {
		if (Firmados<cantFirmas){
			alert("Deben firmar " + cantFirmas + " usuarios para enviar a finalizar");
			return false;
		}
	}
	//Si ya firmaron como minimo la cantidad de usuarios necesarios envio a finalizar
	return true;
}

function PublicarResolucionesEnBloque(id){
	if (document.forms[0].PtosEnSesion.value!="0"){
		alert("No es posible Finalizar dado que hay Asuntos pendientes");
	}else{
		if(document.forms[0].TextoActa.value=="<P>&nbsp;</P>" || document.forms[0].TextoActa.value==""){
			...
			OcultarMenu();
			var pathDB = DirABS();
			window.location.href = pathDB+"/FParametrosPublicarResBloque?Openform&ID="+id+",urlorigen="+document.forms[0].cvRetorno.value+",nroSesion="+document.forms[0].sNroSesion.value;
			}
		}
}
