...
function cargarBandejaResoluciones(){
	...
	document.getElementById('BandejaOrdenes').style.overflow = "hidden";
	document.getElementById('BandejaOrdenes').style.width = "0";
	document.getElementById('BandejaOrdenes').style.heigth = "0";
	
	var path = DirABS();
	
	var strXml = DirABS()+"/GenerarBandejaOrdenXMLNew?OpenAgent";
	//alert(strXml);
	//var strXml = urlBandejaParaFirmar+ "&RestrictToCategory=EXP_"+unidad+usuario+anio+"&Count=3000";
	var mygrid = new dhtmlXGridObject('BandejaOrdenes');
	...
	document.getElementById('BandejaOrdenes').style.overflow = "hidden";
	document.getElementById('BandejaOrdenes').style.width = "100%";
	document.getElementById('BandejaOrdenes').style.heigth = "100%";
	if(navigator.appName == "Netscape")
		document.getElementById('CabezalBandejaOrdenesWrap').style.width = calcularAncho() - 18 +"px";
}

window.onresize = resize;
function resize()
{
	document.getElementById('BandejaOrdenes').style.height = "100%";
	if(grid != null){
		grid.setSizes();
	}
	document.getElementById('BandejaOrdenes').style.width = "100%";
	...
	//else
	//	document.getElementById('CabezalBandejaOrdenesWrap').style.width = calcularAncho() - 20 +"px";
}
...
function actualizarAltos(){
	...
	if(ALTO_MAXIMIZADA<0)
		ALTO_MAXIMIZADA = 0;
	document.getElementById('CabezalBandejaOrdenesWrap').style.height = ""+ALTO_MAXIMIZADA+"px";
}
