...
function cargarBandejaResoluciones(){
	...
	document.getElementById('BandejaActasOrdinarias').style.overflow = "hidden";
	document.getElementById('BandejaActasOrdinarias').style.width = "0";
	document.getElementById('BandejaActasOrdinarias').style.heigth = "0";
	
	var path = DirABS();
	
	var strXml = DirABS()+"/ActasOrdinariasNew?OpenView&count=5000";
	//alert(strXml);
	//var strXml = urlBandejaParaFirmar+ "&RestrictToCategory=EXP_"+unidad+usuario+anio+"&Count=3000";
	var mygrid = new dhtmlXGridObject('BandejaActasOrdinarias');
	...
	document.getElementById('BandejaActasOrdinarias').style.overflow = "hidden";
	document.getElementById('BandejaActasOrdinarias').style.width = "100%";
	document.getElementById('BandejaActasOrdinarias').style.heigth = "100%";
	if(navigator.appName == "Netscape")
		document.getElementById('CabezalBandejaActasOrdinariasWrap').style.width = calcularAncho() - 18 +"px";
}

window.onresize = resize;
function resize()
{
	document.getElementById('BandejaActasOrdinarias').style.height = "100%";
	if(grid != null){
		grid.setSizes();
	}
	document.getElementById('BandejaActasOrdinarias').style.width = "100%";
	...
	//else
	//	document.getElementById('CabezalBandejaActasOrdinariasWrap').style.width = calcularAncho() - 20 +"px";
}
...
function actualizarAltos(){
	...
	if(ALTO_MAXIMIZADA<0)
		ALTO_MAXIMIZADA = 0;
	document.getElementById('CabezalBandejaActasOrdinariasWrap').style.height = ""+ALTO_MAXIMIZADA+"px";
}
