...
if(c!=null&&pt!=null&&(mxClient.IS_GC||mxClient.IS_SF||mxClient.IS_OP)&&(c.scrollLeft>0||c.scrollTop>0))
{
...
var imageNode=null;
if(imgName.substring(imgName.length-3,imgName.length).toUpperCase()=='PNG'&&mxClient.IS_IE6)
{
...
if(this.isVml())
{
this.path.push('qb ',Math.floor(this.translate.x+x1),' ',Math.floor(this.translate.y+y1),' ',Math.floor(this.translate.x+x),' ',Math.floor(this.translate.y+y),
' l ',Math.floor(this.translate.x+x),' ',Math.floor(this.translate.y+y),' ');
}
else
{
...
var points=moveCmd+' '+Math.floor(p0.x-offsetX)+' '+Math.floor(p0.y-offsetY)+' ';
...
var size=0.4;
var b=this.bounds;
var d='m -1,-1 l -1,'+Math.floor(b.height*size)+' c '+Math.floor(b.width*0.3)+','+Math.floor(b.height*0.6)+' '+Math.floor(b.width*0.7)+','+Math.floor(b.height*0.6)+' '+Math.floor(b.width+1)+','+Math.floor(b.height*size)+' l '+Math.floor(b.width+1)+',-1 x e';
...
var size=0.4;
var b=this.bounds;
var d='m '+b.x+','+(b.y-1)+' L '+b.x+','+(b.y+b.height*size)+' Q '+(b.x+b.width*0.5)+','+(b.y+b.height*0.7)+' '+(b.x+b.width+1)+','+(b.y+b.height*size)+' L '+(b.x+b.width+1)+','+(b.y-1)+' z';
...
if(isVml)
{
node.setAttribute('path','m'+Math.floor(pe.x)+','+Math.floor(pe.y)+' l'+Math.floor(pe.x-nx-ny/2)+' '+Math.floor(pe.y-ny+nx/2)+((type!=mxConstants.ARROW_CLASSIC)?'':
' '+Math.floor(pe.x-nx*3/4)+' '+Math.floor(pe.y-ny*3/4))+' '+Math.floor(pe.x+ny/2-nx)+' '+Math.floor(pe.y-ny-nx/2)+' x e');
}
else
{
...
node=document.createElement('DIV');
this.configureHtmlShape(node);
if(mxClient.IS_IE6&&imgName.substring(imgName.length-4,imgName.length)==".PNG")
{
var div=document.createElement('DIV');
...
this.imageNode.setAttribute('transform','scale('+sx+' '+sy+') '+'translate('+dx+' '+dy+')');
}
}
}
};
...
if(!graph.isHtmlLabel(state.cell)&&!mxUtils.isNode(value)&&mxClient.IS_IE&&value!=null)
{
...
if(!this.isEditing()&&(mxClient.IS_OP||mxClient.IS_SF||mxClient.IS_GC||me.getEvent().target!=this.container))
{
...
mxSwimlaneManager.prototype.hasSiblings=function(cell)
{
return true;
};
...
var state=this.graph.view.getState(cell);
var style=(state!=null)?state.style:this.graph.getCellStyle(cell);
return mxUtils.getValue(style,mxConstants.STYLE_HORIZONTAL,1);
}
...
{
if(this.hasSiblings(swimlane))
{
var model=this.getGraph().getModel();
var parent=model.getParent(swimlane);
var horizontal=this.isCellHorizontal(parent);
var childCount=model.getChildCount(parent);
var geo=null;
for(var i=0;i<childCount;i++)
{
...
if(geo!=null)
{
var value=(horizontal)?geo.width:geo.height;
this.resizeSwimlane(swimlane,value,horizontal);
}
else if(geo==null&&this.isBubbling()&&!this.isSwimlaneIgnored(parent))
{
geo=model.getGeometry(swimlane);
var value=(!horizontal)?geo.width:geo.height;
this.resizeParent(parent,value,!horizontal);
...
try
{
if(this.isSiblings()&&this.hasSiblings(swimlane))
{
var childCount=model.getChildCount(parent);
for(var i=0;i<childCount;i++)
{
...
if(child!=swimlane&&!this.isSwimlaneIgnored(child))
{
this.resizeSwimlane(child,value,horizontal);
}
}
}
...
mxSwimlaneManager.prototype.resizeSwimlane=function(swimlane,value,horizontal)
{
var model=this.getGraph().getModel();
var geo=model.getGeometry(swimlane);
if(geo!=null)
...
mxSwimlaneManager.prototype.resizeParent=function(parent,value,horizontal)
{
var graph=this.getGraph();
var model=graph.getModel();
...
{
var tmp=this.getSourcePerimeterPoint(this.previous,current,me);
if(tmp!=null)
{
pt2=tmp;
}
